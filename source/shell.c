...
"history to view all previously typed commands"
};
...
"setenv", // Sets or modifies an environment variable for this shell session
"unsetenv", // Removes an environment variable from the shell
"lang" // change language used for shell prompts
};
...
int shell_usage(char **args){ 
  ...
  { 
    ...
    if (lang_id == 1){
      printf("%s❌ 你输入的指令: %s, 不是 CSEShell 的内建指令.\n", RED, args[1]);
      printf("要查看内建指令列表，请输入 <help>.%s\n", WHITE);
    } else{
      printf("%s❌ The command you gave: %s, is not part of CSEShell's builtin commands.\n", RED, args[1]);
